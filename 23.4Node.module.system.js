...
Circle.prototype.isOverlapped =function (number){
    var d = (Math.sqrt(Math.pow(this.x-number.x,2)  + Math.pow(this.y-number.y,2)));
    if (d ==this.radius + number.radius){
        return 0;
    }
    if (d< this.radius +number.radius){
        return -1;
...
//number ở đây chính là  Circle thứ N được so sánh ở phía sau  dòng 10 13 16
